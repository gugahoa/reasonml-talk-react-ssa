...
let make = () => {
  ...
  let (state, dispatch) =
    React.useReducer(reducer, {user: Types.Anonymous, error: None});
  ...
  let login = (email, password) => {
    ...
    |> Js.Promise.then_(resp =>
         Types.login_resp_decode(resp) |> Js.Promise.resolve
       )
    |> Js.Promise.then_(resp => {
         switch (resp) {
         | Ok((login_resp: Types.login_resp)) =>
           dispatch(SetUser(LoggedIn(login_resp.user)))
  ...
  | (Anonymous, ["loginGraphql"]) =>
    <LoginGraphql onLogin={user => dispatch(SetUser(LoggedIn(user)))} />
  | (LoggedIn({name}), _)
